...
public class RaccoonControllerrrr : MonoBehaviour
{
	void LateUpdate()
    {
        UpdateInput();
		...
		switch (currentState)
		{
			case RaccoonStateee.Falling: 		HandleFalling(); break;
			case RaccoonStateee.Jumping: 		HandleJumping(); break;
			case RaccoonStateee.Walking: 		HandleWalking(); break;
			case RaccoonStateee.Climbing: 		HandleClimbing(); break;
		}
	}
	
	void SetState(RaccoonStateee state)
	{
		ExitState(); 
		currentState = state;
		switch (state)
		{
			case RaccoonStateee.Carrying:
				break;
			case RaccoonStateee.JumpOff:
				animator.SetBool("Climbing", false);
				animator.SetTrigger("Jump");
				Invoke(nameof(SetWalkingState), 1f);
				break;
			case RaccoonStateee.ClimbOver:
				Invoke(nameof(JumpOffForwardPush), 0.2f);
				Invoke(nameof(SetWalkingState), 1.25f);
				animator.SetTrigger("ClimbOver");
				animator.SetBool("Climbing", false);
				break;
			case RaccoonStateee.Falling:
				break;
			case RaccoonStateee.Walking:
				walkingCollider.enabled = true;
				ResetRotationXZ();
				rb.useGravity = true;
				break;
			case RaccoonStateee.Climbing:
	...
	void HandleFalling()
	{
		if (animator.GetBool("isGrounded"))
			SetState(RaccoonStateee.Walking);
	}
	void HandleJumping()
	{
		if (animator.GetBool("isGrounded"))
	...
	void HandleWalking()
	{
		...
		{
			if (CanClimb())
				SetState(RaccoonStateee.Climbing);
			else
				SetState(RaccoonStateee.Jumping);
		}
	...
	void HandleClimbing()
	{
		...
		{
			SetState(RaccoonStateee.JumpOff);
			return;
		}
	...
	void CheckClimbOver()
	{
		...
		if (!Physics.BoxCast(topCenterAndBack, halfExtents, transform.forward, Quaternion.identity, climbingHorizontalDistance / 1.2f))
			SetState(RaccoonStateee.ClimbOver);
	...
	void SetWalkingState() => SetState(RaccoonStateee.Walking);
	void DisableCollider() => walkingCollider.enabled = false;
	[SerializeField] RaccoonStateee currentState;
	...
	public void JumpOffForwardPush() => rb.AddForce(transform.forward * 5f);
